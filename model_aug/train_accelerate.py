...
gradient_accumulation_steps = configs["training"]["text2midi_model"][
...
per_device_train_batch_size = configs["training"]["text2midi_model"][
    "per_device_train_batch_size"
]
save_every = configs["training"]["text2midi_model"]["save_every"]

# Normalize max_train_steps from config (handle strings like "None")
if isinstance(max_train_steps, str):
    if max_train_steps.lower() == "none":
...
tb_writer = (
    SummaryWriter(log_dir=os.path.join(output_dir, "tb")) if with_tracking and accelerator.is_main_process else None
)
...
# Comment out the pretrained model loading for fresh training
# model.load_state_dict(torch.load('./artifacts/pytorch_model.bin', map_location=device))
...
def train_model_accelerate(
    ...
    for epoch in range(starting_epoch, num_epochs):
        ...
        for step, batch in enumerate(dataloader):
            with accelerator.accumulate(model):
                ...
                if use_moe:
                    ...
                    aux_loss = 0
                loss = criterion(
                    outputs.view(-1, outputs.size(-1)), tgt_output.reshape(-1)
                )
                loss += aux_loss
        ...
        if accelerator.is_main_process:
            ...
            if writer:
                writer.add_scalar("train/loss_epoch", result["train_loss"], epoch + 1)
        ...
        if accelerator.is_main_process and checkpointing_steps == "best":
            ...
            if (epoch + 1) % save_every == 0:
                ...
                accelerator.save_state(
                    "{}/{}".format(out_dir, "epoch_" + str(epoch + 1))
                )
